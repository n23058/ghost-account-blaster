...
document.addEventListener('click', async (e) => {
    ...
    if ((target.classList.contains('btn-change') || target.classList.contains('btn-delete')) && serviceName && domain) {
        const data = await chrome.storage.local.get({
        ...
        const searchUrl = `https://www.google.com/search?q=${encodeURIComponent(domain + " " + action)}`;
        ...
        if (target.classList.contains('btn-delete')) {
            if (confirm(`${serviceName} をリストから削除してもよろしいですか？`)) {
                const watchedData = await chrome.storage.local.get({ watched: {} });
                delete watchedData.watched[domain];
                await chrome.storage.local.set(watchedData);
                renderAll();
            }
        }
    }
